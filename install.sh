...
# ── Phase 6: CLAUDE.md identity ───────────────────────────────────────────────
info "Agent identity"

IDENTITY_FILE="$FLEET_DIR/fleet-identity.md"
IDENTITY_LINE="@~/.fleet/fleet-identity.md"

cat > "$IDENTITY_FILE" <<EOF
---
name: fleet-identity
description: Fleet-injected agent identity
metadata:
  type: user
---

Your name is $AGENT_NAME. You are working with $USER_NAME.
EOF
ok "fleet-identity.md written"

if [[ -f "$CLAUDE_MD" ]] && grep -qF "$IDENTITY_LINE" "$CLAUDE_MD"; then
    ok "CLAUDE.md already references fleet-identity — skipping"
else
    print ""
    if [[ -f "$CLAUDE_MD" ]]; then
